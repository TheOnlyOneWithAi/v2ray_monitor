"""V2Ray Monitor API: Telegram identity, subscription import and monitoring."""
...
from .models import Node,Template,MonitorEntitlement,MonitorConfig,Subscription
...
async def entitlement(uid):
    async with Session() as db:
        ...
        count=len((await db.execute(select(Node.id).join(Subscription,Node.subscription_id==Subscription.id).where(Subscription.telegram_id==uid,Subscription.enabled.is_(True),Node.enabled.is_(True)))).scalars().all())
        return e,count
...
async def configs(q:str='',x_telegram_init_data:str|None=Header(default=None)):
    ...
    async with Session() as db:
        rows=(await db.execute(select(Node).join(Subscription,Node.subscription_id==Subscription.id).where(Subscription.telegram_id==uid,Subscription.enabled.is_(True),Node.enabled.is_(True),Node.name.ilike(f'%{q}%')).order_by(Node.name))).scalars().all()
...
async def create_subscription(body:SubscriptionCreate,x_telegram_init_data:str|None=Header(default=None)):
    ...
    url=body.url.strip()
    if not (url.startswith('http://') or url.startswith('https://')): raise HTTPException(400,'Subscription URL must use http or https')
    async with Session() as db:
    ...
    try:
        ...
        async with Session() as db:
            ...
            if row: row.enabled=False; await db.commit()
        raise HTTPException(400,f'Could not import subscription: {exc}') from exc
    if imported < 1:
        async with Session() as db:
            row=await db.get(Subscription,sid)
            if row: row.enabled=False; await db.commit()
        raise HTTPException(400,'No supported VLESS/VMess/Trojan/Shadowsocks nodes were found')
    return {'ok':True,'id':sid,'name':body.name.strip(),'imported':imported}

@app.post('/api/subscriptions/{sid}/sync')
async def resync_subscription(sid:int,x_telegram_init_data:str|None=Header(default=None)):
    uid=validate(x_telegram_init_data); await entitlement(uid)
    async with Session() as db:
        row=await db.get(Subscription,sid)
        if not row or row.telegram_id!=uid or not row.enabled: raise HTTPException(404,'Subscription not found')
    try: imported=await sync_subscription(sid)
    except Exception as exc: raise HTTPException(502,f'Subscription fetch failed: {exc}') from exc
    return {'ok':True,'imported':imported}

@app.delete('/api/subscriptions/{sid}')
async def delete_subscription(sid:int,x_telegram_init_data:str|None=Header(default=None)):
    uid=validate(x_telegram_init_data); await entitlement(uid)
    async with Session() as db:
        row=await db.get(Subscription,sid)
