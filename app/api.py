...
def _safe_node(node: Node) -> dict:
    # Deliberately whitelist fields. Never serialize the ORM object or config_encrypted.
    return {
        "id": node.id,
        "name": node.name,
...
async def nodes() -> dict:
    async with Session() as db:
        rows = (
            await db.execute(
                select(Node).where(Node.enabled.is_(True)).order_by(Node.id)
            )
        ).scalars().all()
...
async def view() -> HTMLResponse:
    async with _TEMPLATE_LOCK:
        async with Session() as db:
            template = (
                await db.execute(select(Template).where(Template.name == "default"))
            ).scalars().first()
            rows = (
                await db.execute(
                    select(Node).where(Node.enabled.is_(True)).order_by(Node.id)
    ...
    # Only these placeholders are substituted. Unknown placeholders are left literal;
    # sensitive values can therefore never be introduced by the renderer.
    rendered = []
    for node in rows:
        values = {
